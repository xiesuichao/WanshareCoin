 ...
 */
@Route(path = FavoriteArouterConstant.FAVORITE_EXCHANGE_MANAGER)
public class ExchangeManagerActivity extends BaseListActivity implements OnRefreshListener, OnLoadMoreListener, MultiItemTypeAdapter.OnItemClickListener, MultiItemTypeAdapter.OnItemLongClickListener {

    private ExchangeManagerAdapter mExchangeManagerAdapter;

    @Override
    protected void initView() {
        super.initView();
        mMyToolbar.setBackgroundColor(getResources().getColor(R.color.color_white));
        mMyToolbar.setTitle(R.string.favorite_exchange_manager);
        ...
        mExchangeManagerAdapter = new ExchangeManagerAdapter(mContext);


        //如果不需要加头部和底部View，直接设置加载normalAdapter
        setAdapter(mExchangeManagerAdapter);
        ...
        //设置条目点击事件
        mExchangeManagerAdapter.setOnItemClickListener(this);
        mExchangeManagerAdapter.setOnItemLongClickListener(this);
    }
    ...
    protected void onRightButton(View view) {
        ...
        ARouter.getInstance().build(FavoriteArouterConstant.FAVORITE_EXCHANGE_SEARCH).navigation(getContext());
    }
}
